...
func init() {
	CreateCollections = append(CreateCollections, func(db *Mongo) error {
		var err error
		collection := db.client.Database(db.config.MongoDatabase).Collection(db.config.MongoCharacteristicCollection)
...
func (this *Mongo) characteristicCollection() *mongo.Collection {
	return this.client.Database(this.config.MongoDatabase).Collection(this.config.MongoCharacteristicCollection)
}
...
func (this *Mongo) SetCharacteristic(ctx context.Context, characteristic models.Characteristic, syncHandler func(models.Characteristic) error) error {
	...
	if err != nil {
		log.Printf("WARNING: error in SetCharacteristic::setSynced %v, will be retried later\n", err)
		return nil
	}
	return nil
}
