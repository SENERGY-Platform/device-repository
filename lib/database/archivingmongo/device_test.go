...
func TestMongoDeviceDeleteBeforeCreate(t *testing.T) {
	...
	ctx, _ = context.WithTimeout(context.Background(), 2*time.Second)
	_, exists, err := m.GetDevice(ctx, "rm")
	if err != nil {
		t.Error(err)
		return
	}
	...
	ctx, _ = context.WithTimeout(context.Background(), 2*time.Second)
	err = m.SetDevice(ctx, model.DeviceInstance{Id: "rm", Name: "foo", Url: "bar", DeviceType: "footype"})
	if err != nil {
		t.Error(err)
		return
	}
	...
	ctx, _ = context.WithTimeout(context.Background(), 2*time.Second)
	_, exists, err = m.GetDeviceByUri(ctx, "rm")
	if err != nil {
		t.Error(err)
		return
	}
...
func TestMongoDeviceTransaction(t *testing.T) {
	skipMsg := `this test needs a prepared clean database with replSet configured:
