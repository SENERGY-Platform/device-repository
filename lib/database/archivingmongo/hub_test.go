...
func TestMongoHubDeleteBeforeCreate(t *testing.T) {
	...
	ctx, _ = context.WithTimeout(context.Background(), 2*time.Second)
	err = m.SetHub(ctx, model.Hub{Id: "rm"})
	if err != nil {
		t.Error(err)
		return
	}
	...
	ctx, _ = context.WithTimeout(context.Background(), 2*time.Second)
	result, err := m.ListHubs(ctx, listoptions.New())
	if err != nil {
		t.Error(err)
		return
	}
