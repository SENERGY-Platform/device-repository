...
func (this *Mongo) ListEndpoints(ctx context.Context, listoptions ...listoptions.ListOptions) (result []model.Endpoint, err error) {
	opt := options.Find()
	filter := bson.M{"removed": bson.M{"$in": bson.A{nil, false}}}
...
func (this *Mongo) RemoveEndpoint(ctx context.Context, id string) error {
	_, err := this.endpointCollection().ReplaceOne(ctx, bson.M{endpointIdKey: id}, bson.M{endpointIdKey: id, "removed": true}, options.Replace().SetUpsert(true))
	return err
}

func (this *Mongo) SetEndpoint(ctx context.Context, endpoint model.Endpoint) error {
	_, err := this.endpointCollection().UpdateOne(ctx, bson.M{endpointIdKey: endpoint.Id}, bson.M{"$set": endpoint}, options.Update().SetUpsert(true))
	return err
}
