...
func TestMongoDeviceTypeDeleteBeforeCreate(t *testing.T) {
	...
	ctx, _ = context.WithTimeout(context.Background(), 2*time.Second)
	_, exists, err := m.GetDeviceType(ctx, "rm")
	if err != nil {
		t.Error(err)
		return
	}
	...
	ctx, _ = context.WithTimeout(context.Background(), 2*time.Second)
	err = m.SetDeviceType(ctx, model.DeviceType{Id: "rm", Name: "foo"})
	if err != nil {
		t.Error(err)
		return
	}
