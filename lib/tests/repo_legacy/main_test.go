...
import (
	...
	testenv2 "github.com/SENERGY-Platform/device-repository/v2/lib/tests/repo_legacy/testenv"
...
func StartController(baseCtx context.Context, wg *sync.WaitGroup, conf configuration.Config) (ctrl *controller.Controller, err error) {
	...
	ctrl, err = controller.New(conf, db, testenv2.VoidProducerMock{}, pc)
	if err != nil {
		db.Disconnect()
...
func testDeviceTypeRead(t *testing.T, conf configuration.Config, expectedDt ...models.DeviceType) {
	...
	req.Header.Set("Authorization", testenv2.Userjwt)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		t.Error(err)
		return
